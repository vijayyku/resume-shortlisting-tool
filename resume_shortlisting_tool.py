...
def build_skill_database(jd_text):
    ...
    jd_text = re.sub(
        r"[-_/]", " ", jd_text
    )  # normalize separators like sap-ui5 → sap ui5
...
for skill in jd_db:
    ...
    for syn in SKILL_MAP.get(skill_lower, []):
        ...
        if (
            syn in resume_words
            or re.search(r"(?<!\w)" + re.escape(syn) + r"(?!\w)", resume_text)
            or syn_norm in resume_norm
        ):
            matched.add(skill)
            break
...
def evaluate(
    ...
    final = sim_score * sim_weight + skill_pct * skill_weight + exp_score * exp_weight
